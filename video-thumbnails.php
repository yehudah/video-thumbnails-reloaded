...
define( 'VIDEO_THUMBNAILS_RL_PATH', dirname(__FILE__) );
define( 'VIDEO_THUMBNAILS_RL_FIELD', '_video_thumbnail' );
define( 'VIDEO_THUMBNAILS_RL_VERSION', '1.0.0' );

// Providers
require_once( VIDEO_THUMBNAILS_RL_PATH . '/php/providers/providers.php' );

// Extensions
require_once( VIDEO_THUMBNAILS_RL_PATH . '/php/extensions/extensions.php' );

// Settings
require_once( VIDEO_THUMBNAILS_RL_PATH . '/php/class-video-thumbnails-settings.php' );

// Class

class Video_Thumbnails_Reloaded {
    ...
    function plugin_admin_notice() {
        echo '<div class="error"><p><strong>Video Thumbnails Reloaded</strong> plugin is a fork (twin brother) of the original Video Thumbnails, you must disable Video Thumbnails to use this plugin - <strong>All your old settings will be saved!!!</strong>.</p></div>';
	...
	function admin_scripts( $hook ) {
		...
		if ( 'tools_page_video-thumbnails-bulk' == $hook ) {
			wp_enqueue_script( 'video-thumbnails-bulk-js', plugins_url( '/js/bulk.js' , __FILE__ ), array( 'jquery' ), VIDEO_THUMBNAILS_RL_VERSION );
			...
			wp_enqueue_style( 'video-thumbnails-bulk-css', plugins_url('/css/bulk.css', __FILE__), false, VIDEO_THUMBNAILS_RL_VERSION );
		}
	}
	...
	function meta_box() {
		...
		if ( isset( $custom[VIDEO_THUMBNAILS_RL_FIELD][0] ) ) $video_thumbnail = $custom[VIDEO_THUMBNAILS_RL_FIELD][0];
		...
		if ( get_post_status() == 'publish' || get_post_status() == 'private' ) {
			if ( isset( $video_thumbnail ) && $video_thumbnail != '' ) {
				echo '<p><a href="#" class="video-thumbnails-reset" data-id="' . $post->ID . '" data-security="' . wp_create_nonce('vtr' ) . '">' . __( 'Reset Video Thumbnail', 'video-thumbnails' ) . '</a></p>';
			} else {
				echo '<p id="video-thumbnails-preview">' . __( 'No video thumbnail for this post.', 'video-thumbnails' ) . '</p>';
				echo '<p><a href="#" class="video-thumbnails-reset" data-id="' . $post->ID . '" data-security="' . wp_create_nonce('vtr' ) . '"">' . __( 'Search Again', 'video-thumbnails' ) . '</a> <a href="#TB_inline?width=400&height=600&inlineId=video-thumbnail-not-found-troubleshooting" class="thickbox" style="float:right;">' . __( 'Troubleshoot', 'video-thumbnails' ) . '</a></p>';
			}
		} else {
			if ( isset( $video_thumbnail ) && $video_thumbnail != '' ) {
				echo '<p><a href="#" class="video-thumbnails-reset" data-id="' . $post->ID . '" data-security="' . wp_create_nonce('vtr' ) . '">' . __( 'Reset Video Thumbnail', 'video-thumbnails' ) . '</a></p>';
	...
	function get_video_thumbnail( $post_id = null ) {
		...
		if( ( $thumbnail_meta = get_post_meta( $post_id, VIDEO_THUMBNAILS_RL_FIELD, true ) ) != '' ) {
		...
		// If the thumbnail isn't stored in custom meta, fetch a thumbnail
			...
			if ( $new_thumbnail != null && !is_wp_error( $new_thumbnail ) ) {
				...
				if ( !update_post_meta( $post_id, VIDEO_THUMBNAILS_RL_FIELD, $new_thumbnail ) ) add_post_meta( $post_id, VIDEO_THUMBNAILS_RL_FIELD, $new_thumbnail, true );
	...
	function ajax_reset_callback() {
		...
		delete_post_meta( $post_id, VIDEO_THUMBNAILS_RL_FIELD );
	...
	function bulk_scanning_page() {
		...
		<div class="wrap">
			...
			<form id="video-thumbnails-bulk-scan-options">
                <?php wp_nonce_field('vtr', 'security'); ?>
...
$video_thumbnails = new Video_Thumbnails_Reloaded();
