...
class Video_Thumbnails_Settings {
	...
	function __construct() {
		...
		register_activation_hook( VIDEO_THUMBNAILS_RL_PATH . '/video-thumbnails.php', array( $this, 'plugin_activation' ) );
		register_deactivation_hook( VIDEO_THUMBNAILS_RL_PATH . '/video-thumbnails.php', array( $this, 'plugin_deactivation' ) );
		// Set current options
	...
	function upgrade_options( $options ) {
		...
		if ( version_compare( $options['version'], VIDEO_THUMBNAILS_RL_VERSION, '<' ) ) {
			$options['version'] = VIDEO_THUMBNAILS_RL_VERSION;
			$options_need_updating = true;
		}
	...
	function admin_scripts( $hook ) {
		if ( 'settings_page_video_thumbnails' == $hook ) {
			wp_enqueue_style( 'video-thumbnails-settings-css', plugins_url( '/css/settings.css', VIDEO_THUMBNAILS_RL_PATH . '/video-thumbnails.php' ), false, VIDEO_THUMBNAILS_RL_VERSION );
			wp_enqueue_script( 'video_thumbnails_settings', plugins_url( 'js/settings.js' , VIDEO_THUMBNAILS_RL_PATH . '/video-thumbnails.php' ), array( 'jquery' ), VIDEO_THUMBNAILS_RL_VERSION );
	...
	function initialize_options() {
		...
		$this->add_text_setting(
			...
			'<a href="#" class="button" data-security="' . wp_create_nonce('vtr') . '" id="vt_detect_custom_field">' . __( 'Automatically Detect', 'video-thumbnails' ) . '</a> ' . __( 'Enter the name of the custom field where your embed code or video URL is stored.', 'video-thumbnails' )
		);
	...
	function options_page() {
		...
		?><div class="wrap">
			...
			<p><?php _e( 'For more detailed instructions, check out the page for <a href="http://wordpress.org/extend/plugins/video-thumbnails-reloaded/">Video Thumbnails on the official plugin directory</a>.', 'video-thumbnails' ); ?></p>
			...
			<div class="video-thumbnails-test closed">
				...
				<div class="video-thumbnails-test-content">
					...
					<div id="provider-test-results" class="hidden">
						<?php
                        $nonce = wp_create_nonce('vtr' );
						foreach ( $video_thumbnails->providers as $provider ) {
							echo '<div data-security="' . $nonce . '" id="' . $provider->service_slug . '-provider-test" class="single-provider-test-results">';
			...
			<div class="video-thumbnails-test closed">
				...
				<div class="video-thumbnails-test-content">
					...
					<p>
                        <?php $nonce = wp_create_nonce('vtr' ); ?>
						<input type="submit" data-security="<?php echo $nonce; ?>" id="test-video-thumbnail-saving-media" class="button-primary video-thumbnails-test-button" value="<?php esc_attr_e( 'Download Test Image', 'video-thumbnails' ); ?>" />
						<input type="submit" data-security="<?php echo $nonce; ?>" id="delete-video-thumbnail-test-images" class="button video-thumbnails-test-button" value="<?php esc_attr_e( 'Delete Test Images', 'video-thumbnails' ); ?>" />
					</p>
					...
					<table class="widefat">
						...
						<tbody>
							...
							<tr>
								...
								<td><?php echo VIDEO_THUMBNAILS_RL_VERSION; ?></td>
								<td></td>
							</tr>
							<tr>
